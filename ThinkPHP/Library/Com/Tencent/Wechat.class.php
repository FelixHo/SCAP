...
class Wechat {
    ...
    public function __construct($token){
        if($token){
            self::auth($token) || exit('auth failed~');
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        /*去除未赋值参数(例如音乐消息的ThumbMediaId其实不是必须参数,但微信文档写必须)
         * 避免XML异常导致微信服务器返回无法提供服务的问题 @hjh*/
     ...
     * @param  string $thumb_media_id 缩略图ID (可选)
     */
     ...
     * [    
     *      0 => 第一条图文信息[标题，说明，全文连接，图片链接]，
     *      1 => 第二条图文信息[标题，说明，全文连接，图片链接]，
     *      2 => 第三条图文信息[标题，说明，全文连接，图片链接]，
     * ]
     */
